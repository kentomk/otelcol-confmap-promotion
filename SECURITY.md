...
The static policy gate fails when the embedded runtime module set or version differs from `policy/runtime-dependencies.tsv`, a module's approved BSD-3-Clause license text hash changes, a workflow Action is not pinned to a full commit SHA, or a tracked text file matches a bounded credential/private-key pattern. It also requires official `govulncheck` v1.6.0, resolved from `PATH` or the Go workspace bin directory, and queries the canonical Go vulnerability database for reachable known vulnerabilities. This networked maintainer check sends module paths under the Go database privacy contract; it does not upload source. A clean result covers the database snapshot and supported static-analysis paths only.
